(() => {
  ...
  const SCROLL_SPEED_GAIN = 0.3;

  const BOAT = { width: 46, height: 78, speed: 480, invulnMs: 1200 };
  ...
  function spawnObst(){
    ...
    state.entities.obst.push({ x, y: -40, w: size, h: size, vy: state.speed });
  }
  ...
  function win(){ state.running = false; showOverlay('¡Has ganado! 🏆', `Has recogido ${WIN_COINS} monedas.`); }
  function gameOver(){ state.running = false; showOverlay('Game Over 💥', `Monedas: ${state.coins}/${WIN_COINS}`); }
